...
ChildPool.prototype.retain = function(processFile, processFileMethod) {
  const _this = this;
  ...
  return convertExecArgv(process.execArgv).then(execArgv => {
    ...
    child.processFileMethod = processFileMethod;

    _this.retained[child.pid] = child;

    child.on('exit', _this.remove.bind(_this, child));

    return initChild(child, child.processFile, child.processFileMethod).then(
      () => {
        return child;
      }
    );
...
const initChild = function(child, processFile, processFileMethod) {
  return new Promise(resolve => {
    child.send(
      { cmd: 'init', value: processFile, method: processFileMethod },
      resolve
    );
  });
};
