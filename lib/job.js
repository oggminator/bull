...
Job.prototype.progress = function(progress, threadId) {
  if (!_.isNil(threadId)) {
    return this.progressThread(progress, threadId);
  } else {
...
Job.prototype.progressThread = function(progress, threadId) {
  if (_.isUndefined(progress)) {
    return this._progress;
  }
  if (_.isUndefined(this.threads[threadId])) {
    this.threads[threadId] = {
      progress: progress
    };
  } else {
    this.threads[threadId].progress = progress;
  }
  var progressThreads = 0;
  _.each(this.threads, function(thread) {
    progressThreads += _.get(thread, 'progress', 0);
  });
  this._progress = _.floor(progressThreads / _.size(this.threads));
  return scripts.updateProgress(this, this._progress);
};
